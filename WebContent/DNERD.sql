...
DROP TRIGGER TRI_dn_friend_f_num;
DROP TRIGGER TRI_dn_goodstrade_t_num;
DROP TRIGGER TRI_dn_member_mem_num;
DROP TRIGGER TRI_dn_message_m_num;
...
DROP SEQUENCE SEQ_dn_friend_f_num;
DROP SEQUENCE SEQ_dn_goodstrade_t_num;
DROP SEQUENCE SEQ_dn_member_mem_num;
DROP SEQUENCE SEQ_dn_message_m_num;
...
CREATE SEQUENCE SEQ_dn_friend_f_num INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_dn_goodstrade_t_num INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_dn_member_mem_num INCREMENT BY 1 START WITH 1;
CREATE SEQUENCE SEQ_dn_message_m_num INCREMENT BY 1 START WITH 1;
...
CREATE OR REPLACE TRIGGER TRI_dn_friend_f_num BEFORE INSERT ON dn_friend
FOR EACH ROW
BEGIN
	SELECT SEQ_dn_friend_f_num.nextval
	INTO :new.f_num
	FROM dual;
END;
...
CREATE OR REPLACE TRIGGER TRI_dn_goodstrade_t_num BEFORE INSERT ON dn_goodstrade
FOR EACH ROW
BEGIN
	SELECT SEQ_dn_goodstrade_t_num.nextval
	INTO :new.t_num
	FROM dual;
END;
...
CREATE OR REPLACE TRIGGER TRI_dn_message_m_num BEFORE INSERT ON dn_message
FOR EACH ROW
BEGIN
	SELECT SEQ_dn_message_m_num.nextval
	INTO :new.m_num
	FROM dual;
END;
...
COMMENT ON COLUMN dn_friend.f_st IS 'ex>
0: 친구 요청 상태
1: 친구 수락 상태
2: 친구 차단 상태(지워 말어??)';
COMMENT ON COLUMN dn_member.age IS '10,20,30,40,50,60(이상)';
COMMENT ON COLUMN dn_message.m_num IS '고유값';
COMMENT ON COLUMN dn_message.s_mem_del_st IS '보낸 사람이 기록을 삭제 했는지 안했는지';
